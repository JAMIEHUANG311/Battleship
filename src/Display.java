	...
	public void initComputerShips() {
		...
		for (int i=0; i<SHIP_LENGTHS.length; i++) {
			do {
				...
				orientation = (dir == 1);
			} while (!grid.addShip(SHIP_LENGTHS[i], new Point(x,y), orientation, SHIP_NAMES[i]));
		}
		
		player[0] = new Computer(grid);  // Computer
...
//			System.out.println("[" + x + ", " + y + "]");
	...
	public void fireOnHuman() {
		...
		if (computer.isInHittingMode()) {
			...
			if ((q != null) && (grid.getFired(q.getX(), q.getY()) == 3)) {
				// ship is sunk
	...
	public Point fireOnHits(Computer computer) {
		...
		if (hitmoves.size() == 0) {
			...
			if (grid.isValid(q) && (grid.getFired(q.getX(), q.getY()) == 0)) {
				grid.fire(q);
				computer.addMove(q);
				return q;
			}
